...
// ============================================================
// POSTGRESQL — Railway + Local compatible
// ============================================================
const pool = new Pool(
  process.env.DATABASE_URL
    ? {
        connectionString: process.env.DATABASE_URL,
        ssl: { rejectUnauthorized: false }     // FIX para Railway
      }
    : {
...
async function generarOrden(client) {
  ...
  const { rows } = await client.query(
    `SELECT COUNT(*) AS total FROM solicitudes WHERE EXTRACT(YEAR FROM fecha_creacion) = $1`,
    [year]
  );
...
app.post('/api/auth/check-email', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'SELECT id, usuario FROM usuarios WHERE correo = $1 AND activo = true',
      [correo]
    );
...
app.post('/api/usuarios/registro', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO usuarios (usuario, correo, password_hash)
       VALUES ($1, $2, $3) RETURNING id, usuario, correo`,
      [usuario, correo, hash]
    );
...
app.post('/api/usuarios/login', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT * FROM usuarios WHERE (usuario = $1 OR correo = $1) AND activo = true`,
      [usuario]
    );
...
app.get('/api/usuarios/:id/perfil', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, usuario, correo, fecha_creacion 
...
app.get('/api/usuarios/:id/ordenes', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT * FROM solicitudes WHERE usuario_id = $1 ORDER BY fecha_creacion DESC`,
      [req.params.id]
    );
...
app.get('/api/dashboard', async (req, res) => {
  try {
    const [statsRes, solRes] = await Promise.all([
      ...
      pool.query(`SELECT s.*, u.usuario AS usuario_registrado
                  FROM solicitudes s
    ...
    return res.json({
      ...
      alta: Number(s.alta),
      media: Number(s.media),
      baja: Number(s.baja),
      recibidas: Number(s.recibidas),
      en_proceso: Number(s.en_proceso),
      completadas: Number(s.completadas),
...
app.get('/api/solicitud/:orden', async (req, res) => {
  try {
    const { rows: sol } = await pool.query(
      `SELECT s.*, u.usuario AS usuario_registrado
       FROM solicitudes s
       LEFT JOIN usuarios u ON s.usuario_id = u.id
       WHERE s.orden = $1`,
      [req.params.orden]
    ...
    if (observacion || cambiado_por) {
      await pool.query(
        `UPDATE historial_estados
         SET observacion=$1, cambiado_por=$2
...
function getOnlineUsers(room) {
  return salas[room]?.usuarios.size || 0;
}
...
wss.on('connection', (ws) => {
    ...
    if (msg.type === 'join') {
      ...
      if (rol === 'asesor') {
        if (salas[room].asesor && salas[room].asesor.readyState === 1) {
          salas[room].asesor.send(JSON.stringify({ type: 'system', text: 'Nueva sesión iniciada.' }));
        ...
        ws.send(JSON.stringify({
          ...
          text: `Conectado como asesor. ${getOnlineUsers(room)} usuario(s).`
        }));
        salas[room].usuarios.forEach(u => {
          if (u.readyState === 1)
            u.send(JSON.stringify({ type: 'asesor-status', online: true, text: `${nombre} está en línea.` }));
        });
  ...
  ws.on('close', () => {
    ...
    if (rol === 'asesor') {
      ...
      salas[room].usuarios.forEach(u => {
        if (u.readyState === 1)
          u.send(JSON.stringify({ type: 'asesor-status', online: false, text: 'El asesor se ha desconectado.' }));
      });
